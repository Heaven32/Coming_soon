...
const DialogsFunction = (props) => {
  const dialogsElements = props.dialogs.map((element) => (
    <DialogItem
      name={element.name}
      key={element.id}
      id={element.id}
  ...
  const messagesElements = props.message.map((element) => (
    <Message message={element.message} key={element.id} id={element.id} />
  ));
